...
export default function ProjectOfficeIssuesPage() {
  ...
  const fetchDeliveryOrders = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        // Only show DOs that are at project office for reporting issues
        const relevantDOs = data.filter((order: any) => 
  ...
  const getDeliveryOrderStatus = (status: string) => {
    const statusConfig: any = {
      at_project_office: "Pending Receipt",
      received_at_project_office: "Received",
      at_road_sale: "Forwarded to Road Sale",
    };
            ...
            <TableBody>
              {issuesList.map((issue) => (
                <TableRow key={issue.id}>
                  ...
                  <TableCell>
                    {issue.deliveryOrder?.party?.name}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Badge variant="outline">
                      {getDeliveryOrderStatus(issue.deliveryOrder?.status)}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => router.push(`/project-office/delivery-orders/${issue.deliveryOrderId}`)}
                      >
                        View DO
                      </Button>
                      {issue.status === "OPEN" && 
                       issue.deliveryOrder?.status === "received_at_project_office" && (
                        <Button
                          variant="default"
                          size="sm"
                          onClick={() => {
            ...
            >
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">DO Status:</span>
                  <Badge variant="outline">
                    {getDeliveryOrderStatus(issue.deliveryOrder?.status)}
                  </Badge>
                </div>
    ...
    <div className="container mx-auto py-6 px-4 sm:px-6 lg:px-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Track and resolve issues for delivery orders at Project Office
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
      ...
      <Dialog open={reportDialogOpen} onOpenChange={setReportDialogOpen}>
        <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Report an issue for a delivery order at Project Office
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
                ...
                <SelectContent>
                  {deliveryOrders.map((order) => (
                    <SelectItem key={order.id} value={order.id}>
                      DO #{order.doNumber} - {order.party?.name} ({getDeliveryOrderStatus(order.status)})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
